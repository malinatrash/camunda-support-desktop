...
Кроссплатформенное desktop-приложение для специалистов поддержки, которые расследуют и восстанавливают процессы Camunda 7 на macOS, Windows и Ubuntu.
...
- поиск активных заявок внутри выбранного процесса по реальным runtime-переменным без ручного ввода ключа;
- фильтруемый каталог переменных процесса с кэшем между запусками приложения и ручным обновлением;
- открытие заявки по клику на любую часть строки и копирование business key отдельным кликом;
- вкладки открытых заявок между разными подключениями с быстрым возвратом;
...
- кэш состояния открытых заявок и фоновое обновление активной заявки каждые 20 секунд;
- объединённое текущее состояние из activity tree, переходов, незавершённой истории, incidents, jobs и external tasks;
- копирование business key и значений переменных; полные сообщения incidents без обрезки;
